...
func (root *semaRoot) queue(addr *uint32, s *sudog, lifo bool, isSyncSema bool) {
	...
	if isSyncSema {
		...
		masked_addr = gc_mask_ptr(unsafe.Pointer(addr))
...
func notifyListWait(l *notifyList, t uint32) {
	...
	s := acquireSudog()
	s.g = getg()
	s.g.waiting_notifier = gc_mask_ptr(unsafe.Pointer(l))
	s.ticket = t
	s.releasetime = 0
	t0 := int64(0)
...
// gc_notifyListNotifyOne updates the notify list to remove deadlocked
// goroutine entries.
func gc_notifyListNotifyOne(l *notifyList, gp *g) *sudog {
	lockWithRank(&l.lock, lockRankNotifyList)
